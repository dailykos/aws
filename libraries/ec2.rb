  ...
  module Aws
    module Ec2
      ...
      def query_private_ip_addresses(interface)
        ...
        ip_addresses = open("http://169.254.169.254/latest/meta-data/network/interfaces/macs/#{mac}/local-ipv4s", options = { proxy: false, read_timeout: 3 }) { |f| f.read.split("\n") }
      ...
      def query_network_interface_id(interface)
        mac = query_mac_address(interface)
        eni_id = open("http://169.254.169.254/latest/meta-data/network/interfaces/macs/#{mac}/interface-id", options = { proxy: false, read_timeout: 3 }, &:gets)
